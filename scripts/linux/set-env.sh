...
if [ $# -lt 1 ]; then
    ...
    echo
    if [ -n "$CRYPTOTEMPLATE_HAS_ENV_SET" ]; then
        echo "Currently-set environment"
        echo "========================="
        echo "Build directory: $CRYPTOTEMPLATE_BUILD_DIR"
        echo "CMake args: $CRYPTOTEMPLATE_CMAKE_ARGS"
    fi
else
    ...
    branch=${branch/\* /}
    builddir_base=~/builds/cryptotemplate/$branch
    case "$buildtype" in
    trace)
    ...
    *)
    ...
    if [ $invalid_buildtype -eq 0 ]; then
        ...
        export PATH="$builddir/libcryptotemplate/bin:$PATH"
        export PATH="$builddir/libcryptotemplate/bin/test:$PATH"
        export PATH="$builddir/libcryptotemplate/bin/bench:$PATH"
        export PATH="$builddir/libcryptotemplate/bin/app:$PATH"
        export PATH="$builddir/libcryptotemplate/bin/examples:$PATH"

        echo "Build directory: $builddir"
        echo "CMake args: $cmake_args"
        echo "PATH envvar: $PATH" 

        export CRYPTOTEMPLATE_BUILD_DIR=$builddir
        export CRYPTOTEMPLATE_CMAKE_ARGS=$cmake_args
        export CRYPTOTEMPLATE_HAS_ENV_SET=1
        export CRYPTOTEMPLATE_SOURCE_DIR=$sourcedir
    else
